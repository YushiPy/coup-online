...
def test_add_and_remove_players():
    ...
    assert (
        "PLAYER1" in match.players
        and "PLAYER2" in match.players
        and "PLAYER1" in match.order
        and "PLAYER2" in match.order
    )
...
def test_start():
    ...
    match = Match("ABCXYZ")
    with pytest.raises(ValueError, match="At least 1 player is required to start."):
        match.start_match()
    ...
    match.start_match()
    assert (
        match.copies_by_card == 3
        and match.status["started"]
        and match.status["current_match_state"] == "waiting_action"
    )
    assert (
        match.players["PLAYER1"].coins == 2 and len(match.players["PLAYER1"].cards) == 2
    )
